...
public class Reader implements Runnable
{
	...
	public void run()
	{
		try
		{
			//Inicializa contador de informações inválidas
			int tryCntr = 0;
			...
			while ( !done )
			{
				...
				if(pkt != null){
					...
					{
						Util.Log("Pacote naum eh meu, reenviar para " + pkt.receiver);
						endpt.btnet.sendPacket(pkt,pkt.receiver);
					}
					else{
				...
				else //Permite 10 tentativas, caso isso ocorra significa que o Dispositivo não está mais no alcance
				{					
